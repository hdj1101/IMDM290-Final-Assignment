...
public class Vam : MonoBehaviour, IGestureCheck
{
    ...
    void Start()
    {
        TtToTtThreshold = 0.05f;
        ItToImcpThreshold = 0.2f;
        MtToMmcpThreshold = 0.2f;
        RtToRmcpThreshold = 0.2f;
        // PtToPmcpThreshold =
        TtToIpipThresholdLess = 0.2f;
        TtToIpipThresholdGreater = 0.075f;
    }
    ...
    {   
        if (
            ...
            Gesture.gen.lefthandpos[8].x < Gesture.gen.righthandpos[8].x &&
            Gesture.gen.lefthandpos[12].x < Gesture.gen.righthandpos[12].x &&
